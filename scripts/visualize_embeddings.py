...
def visualize_embeddings():
    """
    Main function to create optimized embeddings visualizations.
    """
    ...
    embeddings, documents, metadatas, ids = data
    
    # Load categories
    categories = load_categories()
    
    # If no categories file, create default categories
    if not categories:
        print("No categories found. Creating default categories...")
        categories = ['Unknown'] * len(embeddings)
    
    # Ensure categories match embeddings length
    if len(categories) != len(embeddings):
        print(f"Warning: Categories length ({len(categories)}) doesn't match embeddings length ({len(embeddings)})")
        categories = categories[:len(embeddings)] + ['Unknown'] * max(0, len(embeddings) - len(categories))
    ...
    for method in methods:
        ...
        embeddings_2d = perform_dimensionality_reduction(embeddings, method=method)
        
        # Create standard plot
        title = f'{method.upper()} Visualization of APOD Embeddings'
        create_standard_plot(
            embeddings_2d, 
            categories, 
            title,
            f'apod_visualization_{method}.png'
        )
        ...
        create_enhanced_plot(
            ...
            categories, 
            title,
            f'apod_visualization_{method}_enhanced.png'
        )
    
    total_time = time.time() - total_start_time
    print(f"\n--- All visualizations completed in {total_time:.2f} seconds ---")
